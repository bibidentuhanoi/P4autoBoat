"""STRAIGHT 10 s: the lake machinery running a straight-only profile.

precheck 2 s -> straight 10 s at the chosen throttle, rudder centred -> stop
...
class StraightProfileTest(unittest.TestCase):
    ...
    def test_profile_is_precheck_straight_stop_17s(self):
        ph = T.lake_id_phases(0.2, 0.0, 'NA', profile='straight')
        self.assertEqual([p[0] for p in ph], ['precheck', 'straight', 'stop'])
        self.assertEqual(sum(p[1] for p in ph), 17.0)
        self.assertEqual(sum(p[1] for p in ph if p[2] > 0), 10.0)
        self.assertEqual((ph[1][2], ph[1][3]), (0.2, 0.0))
        self.assertEqual(T.lake_id_profile_s(ph), 17.0)
        self.assertEqual(T.lake_id_powered_s(ph), 10.0)
...
class StraightRunTest(LakeBase):
    ...
    def test_full_run_completes_in_its_own_folder(self):
        ...
        self.assertGreater(span, 9.5)
        self.assertLess(span, 10.1)
        self.assertEqual(summary['schema'], 'straight_run_summary_v1')
        self.assertEqual(summary['settings']['profile'], 'straight')
        self.assertEqual(summary['settings']['profile_s'], 17.0)
        self.assertEqual(summary['settings']['powered_s'], 10.0)
        self.assertEqual(len(summary['straight']['yaw_bins']), 20)
    ...
    def test_status_and_result_carry_the_profile(self):
        ...
        self.assertEqual(s['profile_s'], 17.0)
