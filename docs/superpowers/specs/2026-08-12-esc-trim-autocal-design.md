...
```
`control_arbiter` already keeps `throttle` and `rudder` separate before mixing. **The trim is looked up on the ORIGINAL commanded throttle, not on `(L+R)/2` of the post-mix pair** — an important correction found during implementation (`esc_trim.c::esc_trim_mix`, `control_arbiter.c`). `(L+R)/2` recovers the common throttle losslessly *only while neither side is clamped*; once the pilot's `throttle ± rudder` mix saturates at 0 or 1 (a hard turn near full throttle), the average no longer equals the intended throttle and the pilot's turn no longer cancels — reading the trim off it would both look up the wrong table entry and distort the turn. So the arbiter now exposes the pre-mix `throttle`/`rudder` in `control_decision_t`, and `esc_trim_mix(throttle, rudder, …, &left, &right)` runs in `control_apply_decision` (before `esc_driver_set_throttle`): it re-derives `base_left/right` from the original command, looks up `trim = esc_trim_lookup(clamp01(throttle))`, and applies `left -= trim/2; right += trim/2` **within the actuator headroom that remains after the pilot's turn** — the differential is bounded so it can never push a side outside `[0,1]` or claw back steering the pilot already has, and it decays to zero when there is no headroom left (pilot steering always wins at saturation). Then the existing `esc_map` floor/curve per side, last. Trim fixes the permanent bias on the common component; the pilot's turn stays fully intentional on top; the floor mapping stays last. **During calibration itself the saved-table lookup is bypassed** (`s_calibrating` gates `esc_trim_mix` off in the drive path — the routine is *learning* the table, producing the balance directly) while the floor mapping still applies (motors always need the floor to spin).
