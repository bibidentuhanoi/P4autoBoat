 ...
 * -------------------------------------------------------------------------*/
static void upstream_cb(uint32_t msg_id, const uint8_t *data, size_t data_len, void *user)
{
    (void)msg_id;
    (void)user;
    s_last_upstream_us = esp_timer_get_time();
 ...
 * -------------------------------------------------------------------------*/
static void init_status_cb(uint32_t msg_id, const uint8_t *data, size_t data_len, void *user)
{
    (void)msg_id;
    (void)user;
    if (!data || data_len < 1) {
...
esp_err_t espnow_transport_probe(uint32_t timeout_ms)
{
     ...
     * esp_hosted >=2.12.4 callback signature takes a trailing user-context
     * pointer (see idf_component.yml's esp_hosted pin comment); unused here,
     * both sites pass NULL. */
